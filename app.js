...
async function loadPostImages() {
  const imgs = document.querySelectorAll('.post-image[data-fid]');
  for (const img of imgs) {
    const url = await resolveFileUrl(img.dataset.fid);
    if (url) img.src = url;
  ...
  try {
    ...
    for (const p of allPosts) {
      ...
      if (p.photo && p.photo.length > 0) {
        html += `<img class="post-image" data-fid="${p.photo[p.photo.length - 1].file_id}" alt="صورة" loading="lazy">`;
      }
      if (caption) {
        html += `<div class="post-caption">${caption}</div>`;
      }
      html += `<div class="post-date">${date}</div></div>`;
    }
